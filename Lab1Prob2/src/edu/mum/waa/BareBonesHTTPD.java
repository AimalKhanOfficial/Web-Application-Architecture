...
public class BareBonesHTTPD extends Thread {
	...
	private void processRequest(BBHttpRequest httpRequest, BBHttpResponse httpResponse) {
		if(httpRequest.getUri().toString().length() > 0) {
			Map<String, iPages> configXML = new HashMap<>();
			configXML.put("/welcome.web", new Welcome());
			configXML.put("/contacts.web", new Contacts());
			
			if(configXML.containsKey(httpRequest.getUri())) {
				httpResponse = configXML.get(httpRequest.getUri()).getPageAndResponse(httpRequest, httpResponse);
			}
			else {
				httpResponse.setStatusCode(404);
				httpResponse.setMessage("No such file Exists!");
			}
		}
		else {
			httpResponse.setStatusCode(404);
